...
namespace IAmProductive.AppUtil
{
    public class AppUtil
    {
            ...
            try{
                ...
                if (token[1].ToString().Equals("0") == false)
                {
                    if (stringBuilder.ToString().Contains("h")) // if h previously appended then also append the 'and'
                        stringBuilder.Append(" ");// "and" put if want
                ...
                {    //Exclude seconds
                    ...
                    {  //if time is only consisit of seconds then returen the seconds
                        stringBuilder.Append(token[2] + "s");
                    }
        ...
        public static List<string> ChartsTypesList()
        {
           ...
           // chartsCategory.Add(Constants.AppConstant.PayramidChartTypeLable);
